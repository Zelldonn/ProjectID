...
public static class SpringUtils
{
    ...
    public static void UpdateDampedSpringMotionVector3(
        ref Vector3 pPos,           // position value to update
        ref Vector3 pVel,           // velocity value to update
        Vector3 equilibriumPos, // position to approach
        in tDampedSpringMotionParams springParams)
    {
        float[] pos = { pPos.x, pPos.y, pPos.z };
        float[] vel = { pVel.x, pVel.y, pVel.z };
        float[] eq = { equilibriumPos.x, equilibriumPos.y, equilibriumPos.z };


        for (int i = 0; i < 3; i++)
        {
            float oldPos = pos[i] - eq[i]; // update in equilibrium relative space
            float oldVel = vel[i];

            (pos[i]) = oldPos * springParams.m_posPosCoef + oldVel * springParams.m_posVelCoef + eq[i];
            (vel[i]) = oldPos * springParams.m_velPosCoef + oldVel * springParams.m_velVelCoef;
        }
        pPos.Set(pos[0], pos[1], pos[2]);
        pVel.Set(pVel[0], pVel[1], pVel[2]);
    }
}